        ...
        <div className="flex items-center justify-between h-20">
          <Link to="/" onClick={() => window.scrollTo(0, 0)} className="flex items-center">
            <motion.img
              ...
              whileHover={{ scale: 1.05, rotate: [0, -2, 2, 0], filter: 'brightness(0) invert(1) drop-shadow(0 0 8px rgba(212, 168, 75, 0.4))' }}
              transition={{ type: 'spring', stiffness: 400, damping: 25, rotate: { duration: 0.4 } }}
            />
          </Link>
          <nav className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              link.isRoute ? (
                <Link key={link.name} to={link.href} onClick={() => window.scrollTo(0, 0)}
                ...
                <a key={link.name} href={isHomePage ? link.href : '/' + link.href}
          ...
          <div className="flex items-center gap-4">
            ...
            {user && (
              <Link to="/wishlist" className="hidden md:flex p-2 text-parchment/80 hover:text-pepper-red transition-colors" title="My Wishlist">
            ...
            {!isLoading && user && (
              <div className="hidden md:flex items-center gap-3">
                {isAdmin && (
                  <Link to="/admin" className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-gold hover:text-gold/80 border border-gold/40 rounded-md hover:border-gold/60 transition-colors" title="Admin Panel">
                    <Shield className="w-4 h-4" />
                    <span>Admin</span>
                  </Link>
                )}
                <button onClick={() => signOut()} className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-parchment/80 hover:text-gold border border-parchment/30 rounded-md hover:border-gold/50 transition-colors" title="Sign out">
        ...
        {isOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            ...
            <nav className="container mx-auto px-4 py-6 flex flex-col gap-4">
              {navLinks.map((link) => (
                link.isRoute ? (
                  <Link key={link.name} to={link.href} onClick={() => { window.scrollTo(0, 0); setIsOpen(false); }}
