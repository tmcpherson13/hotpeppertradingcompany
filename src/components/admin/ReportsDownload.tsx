...
// Credit Review Report content
const CREDIT_REVIEW_REPORT = `# Credit Review Request — January 3, 2026
...
export function ReportsDownload() {
  const handleDownload = () => {
    const blob = new Blob([CREDIT_REVIEW_REPORT], { type: 'text/markdown' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = 'credit-review-report-2026-01-03.md';
  ...
  return (
    <div className="space-y-6">
      <Card className="bg-parchment border-ink/20">
        <CardHeader>
          <div className="flex items-start gap-4">
            <div className="p-3 bg-tyrian/10 rounded-lg">
              <AlertTriangle className="w-6 h-6 text-tyrian" />
            </div>
            <div className="flex-1">
              <CardTitle className="font-heading text-lg text-ink">
                Credit Review Report — January 3, 2026
              </CardTitle>
              <CardDescription className="font-body text-ink/60 mt-1">
                Documentation of 5 execution standards violations across 2 failed remediation attempts
              </CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-parchment-dark/30 p-3 rounded border border-ink/10">
                <div className="font-heading text-2xl text-tyrian">5</div>
                <div className="font-body text-xs text-ink/60 uppercase tracking-wider">Violations</div>
              </div>
              <div className="bg-parchment-dark/30 p-3 rounded border border-ink/10">
                <div className="font-heading text-2xl text-tyrian">2</div>
                <div className="font-body text-xs text-ink/60 uppercase tracking-wider">Failed Attempts</div>
              </div>
              <div className="bg-parchment-dark/30 p-3 rounded border border-ink/10">
                <div className="flex items-center gap-1">
                  <Calendar className="w-4 h-4 text-ink/40" />
                  <span className="font-heading text-sm text-ink">Jan 3, 2026</span>
                </div>
                <div className="font-body text-xs text-ink/60 uppercase tracking-wider mt-1">Incident Date</div>
              </div>
            </div>

            {/* Violations List */}
            <div className="bg-parchment-dark/20 p-4 rounded border border-ink/10">
              <h4 className="font-heading text-xs uppercase tracking-wider text-ink/70 mb-3">
                Documented Violations
              </h4>
              <ul className="space-y-2 font-body text-sm text-ink/80">
                <li className="flex items-start gap-2">
                  <span className="text-tyrian font-bold">1.</span>
                  <span>Multi-Part Request Protocol</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-tyrian font-bold">2.</span>
                  <span>Visual Integrity Scan</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-tyrian font-bold">3.</span>
                  <span>Verification Requirements</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-tyrian font-bold">4.</span>
                  <span>Self-Verification Mandate</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-tyrian font-bold">5.</span>
                  <span>Anti-Repetition Protocol</span>
                </li>
              </ul>
            </div>
            ...
            <p className="font-body text-xs text-ink/50 text-center">
              Download the full report for submission to Lovable Support
            </p>
          </div>
        </CardContent>
      </Card>
